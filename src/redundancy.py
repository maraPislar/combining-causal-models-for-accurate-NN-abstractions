...
import argparse
from causal_models import RedundantSummingCausalModels
from utils import redundancy_input_sampler, save_results,visualize_per_trained_model
...
def tokenizePrompt(input):
    ...
    prompt = f"{input['X1']}+{input['X2']}-{input['X3']}+{input['Y']}+{input['Z']}="
...
def main():
    ...
    redundancy_family = RedundantSummingCausalModels()
    
    for train_id, model_info in redundancy_family.causal_models.items():
        ...
        training_counterfactual_data = model_info['causal_model'].generate_counterfactual_dataset(
            ...
            args.batch_size,
            device="cuda:0",
            sampler=redundancy_input_sampler,
            ...
            for layer in range(model_config.n_layer):
                ...
                for epoch in train_iterator:
                    ...
                    for step, inputs in enumerate(epoch_iterator):
                        ...
                        if inputs["intervention_id"][0] == 2:
                            _, counterfactual_outputs = intervenable(
                                ...
                                subspaces=[
                                    ...
                                    [[_ for _ in range(low_rank_dimension, low_rank_dimension*2)]] * args.batch_size
                                ]
                            )

                        elif inputs["intervention_id"][0] == 0:
                            _, counterfactual_outputs = intervenable(
                                {"input_ids": inputs["input_ids"]},
                                [
                                    {"input_ids": inputs["source_input_ids"][:, 0]},
                        ...
                        elif inputs["intervention_id"][0] == 1:
                            _, counterfactual_outputs = intervenable(
                                {"input_ids": inputs["input_ids"]},
                                [
                                    {"input_ids": inputs["source_input_ids"][:, 0]},
                ...
                for test_id, test_model_info in redundancy_family.causal_models.items():
        ...
        for experiment_id in [64, 128, 256, 768, 4608]:
            visualize_per_trained_model(args.results_path, save_dir_path, model_config.n_layer, train_id, experiment_id, redundancy_family)
