...
def main():
    ...
    parser.add_argument('--results_path', type=str, default='empirical_results/', help='path to the results folder')
        ...
        for token in [0,1,2,3,4,5]:
            for layer in range(model_config.n_layer):
                ...
                for test_id, test_model_info in arithmetic_family.causal_models.items():
                    ...
                    save_results(args.results_path, report, layer, tokenizePrompt, train_id, test_id)
        
        for experiment_id in [0,1,2,3,4,5]:
