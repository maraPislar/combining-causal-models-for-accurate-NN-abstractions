...
class CausalModel:
    ...
    def generate_counterfactual_dataset_on_bases(
            self,
            size,
            intervention_id,
            batch_size,
            ...
            while count < size:
                ...
                if filter is None or filter(intervention):
                    for _ in range(batch_size):
                        ...
                        sources = []
                        source_dic = {}
                        # refactor a little bit here because of the predefined sources and bases
                        for var in self.variables:
                            if var not in intervention:
                                continue
    ...
    def generate_counterfactual_pairs(
        ...
        for key, base_input, source_input in [("base_source", base, source), ("source_base", source, base)]:
            ...
            if filter is None or filter(intervention):
                ...
                for var in self.variables:
                    if var not in intervention:
                        continue
                    sources.append(inputFunction(source_input))
                    source_dic[var] = source_input

                for _ in range(maxlength - len(sources)):
                    sources.append(torch.zeros(inputFunction(sampler()).shape))

                example["labels"] = outputFunction(self.run_interchange(base_input, source_dic)).to(device)
                example["base_labels"] = outputFunction(self.run_forward(base_input)).to(device)
                example["input_ids"] = inputFunction(base_input).to(device)
                example["source_input_ids"] = torch.stack(sources).to(device)
